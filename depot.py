...
def main():
    ...
    match selection:
        case "1" | "paint" | "one":
            try:
                length, walls = input('What is the length adn iwtPlease input values in forms of "1x2" inches: ').replace(" ", "").split("x"), float(input("How many walls will you be painting? "))
        ...
        case "2" | "flooring" | "two":
            try:
                length, flooring = input('Please input values in forms of "1x2" inches: ').replace(" ", "").split("x"), input("What kind of flooring would you like? (Please enter an integer)\n1. 10in x 10in Tiles\n2. Carpet\nSelection: ")
